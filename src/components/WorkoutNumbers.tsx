...
import { getValidExercises, getValidWorkouts } from "@/functions";
import { useState } from "react";

const WorkoutNumberCard = ({ label, value, icon = null, obs = null }) => {
  return (
    <div className="flex flex-col items-center justify-center w-full h-full p-4">
      <div className="font-bold text-3xl">{value}</div>
      <div className="font-semibold">{label}</div>
      {obs && <div className="text-xs">{obs}</div>}
    </div>
  );
};
...
const WorkoutNumbers = ({ workouts }) => {
  ...
  const workoutPercent = Math.round((workoutsCount * 100) / workouts.length);
